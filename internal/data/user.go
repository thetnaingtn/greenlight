...
type User struct {
	ID         int64     `json:"id"`
	CreactedAt time.Time `json:"created_at"`
...
func (m UserModel) Insert(user *User) error {
	...
	err := m.DB.QueryRowContext(ctx, query, args...).Scan(&user.ID, &user.CreactedAt, &user.Version)
	if err != nil {
		switch {
...
func (m UserModel) GetByEmail(email string) (*User, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, args...).Scan(&user.ID, &user.CreactedAt, &user.Name, &user.Email, &user.Password.hash, &user.Activated, &user.Version)
	if err != nil {
		switch {
...
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, args...).Scan(
		&user.ID, &user.CreactedAt, &user.Name, &user.Email, &user.Password.hash, &user.Activated, &user.Version,
	)
